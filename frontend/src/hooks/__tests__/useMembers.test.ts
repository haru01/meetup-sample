...
describe('useMembers フック', () => {
  ...
  it('listMembers でメンバー一覧を取得する', async () => {
    const members = [
      { id: 'm1', communityId: 'c1', accountId: 'a1', role: 'OWNER', status: 'ACTIVE', createdAt: '' },
    ]
  ...
  it('joinCommunity でコミュニティに参加する', async () => {
    const member = { id: 'm1', communityId: 'c1', accountId: 'a1', role: 'MEMBER', status: 'ACTIVE', createdAt: '' }
    ...
    expect(mockPost).toHaveBeenCalledWith('/communities/c1/members', { accountId: 'a1' })
  })
  ...
  it('approveMember でメンバーを承認する', async () => {
    ...
    expect(success).toBe(true)
    expect(mockPatch).toHaveBeenCalledWith('/communities/c1/members/m1/approve', {})
  })
  ...
  it('rejectMember でメンバーを拒否する', async () => {
    ...
    expect(success).toBe(true)
    expect(mockPatch).toHaveBeenCalledWith('/communities/c1/members/m1/reject', {})
  })
})
