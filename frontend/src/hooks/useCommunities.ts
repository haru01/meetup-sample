...
export const useCommunities = () => {
  ...
  const createCommunity = useCallback(async (data: CreateCommunityRequest): Promise<Community | null> => {
    setLoading(true)
    setError(null)
    const result = await apiClient.post<CommunityResponse>('/communities', data)
    if (result.ok) {
      setLoading(false)
  ...
  const getMyCommunities = useCallback(async () => {
    setLoading(true)
    setError(null)
    const result = await apiClient.get<ListCommunitiesResponse>('/communities/me')
    if (result.ok) {
      setCommunities(result.data.communities)
    } else {
