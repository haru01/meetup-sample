...
import type { CommunityMember, ListMembersResponse, MemberResponse } from '../lib/types'
...
export const useMembers = () => {
  ...
  const joinCommunity = useCallback(async (communityId: string, accountId: string): Promise<boolean> => {
    setLoading(true)
    setError(null)
  ...
  const leaveCommunity = useCallback(async (communityId: string, memberId: string): Promise<boolean> => {
    setLoading(true)
    setError(null)
  ...
  const approveMember = useCallback(async (communityId: string, memberId: string): Promise<boolean> => {
    setLoading(true)
    setError(null)
  ...
  const rejectMember = useCallback(async (communityId: string, memberId: string): Promise<boolean> => {
    setLoading(true)
    setError(null)
    const result = await apiClient.patch<MemberResponse>(
