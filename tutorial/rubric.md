...
- 最低到達: 全軸 L2（基礎的な使い方を理解し、一人で試行できる）
- 推奨到達: 主要軸で L3（実務のプロジェクトに適用できる）
...
| **L4 — 応用** | CLAUDE.md の階層構造（グローバル / プロジェクト / ディレクトリ）を設計できる。skills でオンデマンドのワークフローを定義し、CLAUDE.md の肥大化を防げる。subagent 定義（`.claude/agents/`）でレビューや調査を自動化できる |
...
| 日程 | 時間帯 | 対象軸 | テーマ | 主な活動 |
|------|--------|--------|--------|----------|
| **Day 1** | 午前 | 1, 5 | **触れる・探索する** | Claude Code 起動、モード切替、コードベース質問、Plan Mode で調査 |
| **Day 1** | 午後 | 2 | **意図を伝える** | ユーザーストーリーで機能追加を依頼、フロント+バックの変更を実現 |
| **Day 2** | 午前 | 3, 7 | **レビューする** | diff を読む、テストで検証、計画の抜け漏れを指摘、失敗パターンを体験 |
| **Day 2** | 午後 | 4 | **安定化する** | CLAUDE.md にルール追加、hooks 設定、過剰な CLAUDE.md の弊害を体験 |
| **Day 3** | 午前 | 6 | **使い分ける** | 伴奏 vs 委託の実践、サブエージェント活用、/clear によるコンテキスト管理 |
| **Day 3** | 午後 | — | **自分の環境への適用計画** | 自分のプロジェクトで何を試すか計画を立てる（CLAUDE.md 設計、ワークフロー選定、最初に取り組むタスクの特定） |
...
| 総合レベル | 目安 |
|------------|------|
| **Beginner** | 全軸 L1-L2。AI ツールの基本操作ができる |
| **Intermediate** | 過半数の軸で L3。実務で AI 駆動開発を実践できる |
| **Advanced** | 全軸 L3 以上、複数軸で L4。チームに AI 駆動開発を導入・指導できる |
