...
| 日程 | 時間帯 | 対象軸 | テーマ | 主な活動 | 到達目標 |
|------|--------|--------|--------|----------|----------|
| **Day 1** | 午前 | 1, 5 | **触れる・探索する** | Claude Code 起動、モード切替、コードベース質問、Plan Mode で調査 | L2: モードの違いを説明でき、質問で学べる |
| **Day 1** | 午後 | 2 | **意図を伝える** | ユーザーストーリーで機能追加を依頼、フロント+バックの変更を実現 | L2→L3: 具体的なコンテキストを含む依頼ができる |
| **Day 2** | 午前 | 3, 7 | **レビューする** | diff を読む、テストで検証、計画の抜け漏れを指摘、失敗パターンを体験 | L2→L3: diff を確認し、問題を指摘・軌道修正できる |
| **Day 2** | 午後 | 4 | **安定化する** | CLAUDE.md にルール追加、hooks 設定、過剰な CLAUDE.md の弊害を体験 | L2→L3: 設定ファイルでルールを永続化できる |
| **Day 3** | 午前 | 6 | **使い分ける** | 伴奏 vs 委託の実践、サブエージェント活用、/clear によるコンテキスト管理 | L2→L3: 場面に応じて伴奏と委託を選択できる |
| **Day 3** | 午後 | — | **自分の環境への適用計画** | 自分のプロジェクトで何を試すか計画を立てる（CLAUDE.md 設計、ワークフロー選定、最初に取り組むタスクの特定） | 全軸 L2 確認、L3 への道筋を計画 |
